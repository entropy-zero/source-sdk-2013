...
class C_AI_WeaponLaserUser : public BASE_NPC
{
	DECLARE_CLASS_NOFRIEND( C_AI_WeaponLaserUser, BASE_NPC );

public:
	C_AI_WeaponLaserUser()
	{
	...
	virtual C_BaseAnimating *GetActiveLaserWeapon() { return this->GetActiveWeapon(); }
...
//-----------------------------------------------------------------------------
template <class BASE_NPC>
void C_AI_WeaponLaserUser<BASE_NPC>::OnDataChanged( DataUpdateType_t type )
{
...
//-----------------------------------------------------------------------------
template <class BASE_NPC>
void C_AI_WeaponLaserUser<BASE_NPC>::Simulate()
{
	BaseClass::Simulate();
	...
	if ( pEZ2Player && m_hGunLaser && !m_hGunLaser->IsDormant() )
	{
		...
		if ( GetActiveLaserWeapon() )
		{
			GetActiveLaserWeapon()->GetAttachment( m_nGunLaserAttachment, vecLaserOrigin );
		}
