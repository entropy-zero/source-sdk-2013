...
void CColorCorrectionExclude::OnChangeExcludeTarget()
{
	...
	{
		if ( m_hOldExcludeTarget )
		{
			// Beams need to network their nodraw state so that the client knows if they're on or not
			CBeam *pBeam = dynamic_cast<CBeam *>(m_hOldExcludeTarget.Get());
			if ( pBeam )
			{
				pBeam->SetNetworkNodraw( false );
			}
		}
		...
		{
			// Beams need to network their nodraw state so that the client knows if they're on or not
			CBeam *pBeam = dynamic_cast<CBeam *>(m_hExcludeTarget.Get());
			if ( pBeam )
			{
				pBeam->SetNetworkNodraw( true );
			}
		}
