...
void CNPC_Conscript::Precache()
{
	...
	if ( IsGasmask() )
	{
		PrecacheScriptSound( "NPC_Conscript.Radio_On" );
		PrecacheScriptSound( "NPC_Conscript.Radio_Off" );
	}
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
float CNPC_Conscript::PlayAutoGeneratedSoundScene( const char *soundname, float flDelay, AI_Response *response, IRecipientFilter *filter )
{
	if ( IsGasmask() && response && response->GetType() == ResponseRules::RESPONSE_SPEAK && gpGlobals->curtime - GetLastDamageTime() > 0.3f )
	{
		// Convert speech to choreo sentences that use radio on/off sounds
		char szSentence[256];
		V_snprintf( szSentence, sizeof( szSentence ), "#NPC_Conscript.Radio_On %s NPC_Conscript.Radio_Off; %s", soundname, soundname );

		return PlayChoreoSentenceScene( szSentence, flDelay, response, filter );
	}

	return BaseClass::PlayAutoGeneratedSoundScene( soundname, flDelay, response, filter );
}
