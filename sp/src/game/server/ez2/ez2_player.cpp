...
#define CLOAK_INVISIBLE_TO_NPCS_DIST			1500
...
void CEZ2_Player::Touch( CBaseEntity *pOther )
{
	...
	if ( g_hStealthManager && !(pOther->GetFlags() & FL_OBJECT) && !(GetFlags() & FL_NOTARGET) )
	{
...
void CEZ2_Player::ModifyOrAppendCriteria(AI_CriteriaSet& criteriaSet)
{
	...
	float lastSpokeTime = LastTimePlayerTalked();
	if ( *lastSpokeConcept )
	{
		float lastSpokeConceptTime = GetTimeSpokeConcept( lastSpokeConcept );
		if ( lastSpokeConceptTime > lastSpokeTime )
			lastSpokeTime = lastSpokeConceptTime;
	}
